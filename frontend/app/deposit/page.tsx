...
export function useTokenBalances() {
  ...
  async function fetchWithCovalent(address) {
    ...
    try {
      const response = await axios.get(`https://api.covalenthq.com/v1/${COVALENT_CHAIN_ID}/address/${address}/balances_v2/`, {
        headers: {
      ...
      if (response.data?.data?.items) {
        const tokenItems = response.data.data.items.filter((item) => parseFloat(item.balance) > 0);
  ...
  async function fetchWithBaseTokenList(address) {
    try {
      const tokenListResponse = await axios.get("https://raw.githubusercontent.com/base-org/token-list/main/build/base.tokenlist.json");

      const tokenList = tokenListResponse.data.tokens.filter((token) => token.chainId === COVALENT_CHAIN_ID);
      ...
      <main className="flex-grow pt-28 pb-20">
        <div className="max-w-md md:max-w-2xl mx-auto px-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 overflow-hidden">
            ...
            {ready && !authenticated && (
              <div className="p-8">
                <div className="text-center py-8 px-4">
                  ...
                  <p className="text-gray-600 dark:text-gray-300 mb-8">Connect your wallet to see your available tokens and make a deposit</p>
                  <button
              ...
              <div className="p-8">
                <div className="bg-orange-50 dark:bg-orange-900/20 rounded-xl p-6 max-w-2xl mx-auto">
                  <div className="flex items-start mb-4">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-900 dark:text-white text-lg">Wrong Network Detected</h3>
            ...
            {/* No Tokens Found */}
            {ready && authenticated && networkCorrect && !loadingTokens && tokens.length === 0 && depositStep === 0 && (
              <div className="p-8">
                <div className="text-center py-10">
                  ...
                  <h2 className="text-2xl font-semibold mb-4 text-gray-900 dark:text-white">No Tokens Found</h2>
                  <p className="text-gray-600 dark:text-gray-300 mb-2">We couldn't find any tokens in your wallet on Base network.</p>
            ...
            {ready && authenticated && networkCorrect && !loadingTokens && tokens.length > 0 && depositStep === 0 && (
              <div className="p-8">
                <h2 className="text-xl font-semibold mb-2 text-gray-900 dark:text-white">Choose a Token</h2>
                <p className="text-gray-600 dark:text-gray-400 mb-6">Select which token you want to deposit:</p>
            ...
            {ready && authenticated && networkCorrect && depositStep === 1 && selectedToken && (
              <div className="p-8">
                <button onClick={() => setDepositStep(0)} className="flex items-center text-blue-600 dark:text-blue-400 mb-6 hover:underline">
                  <ArrowLeft className="mr-1" size={16} /> Back to tokens
                </button>
                <h2 className="text-xl font-semibold mb-6 text-gray-900 dark:text-white">Enter Amount</h2>
                <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 mb-6">
                  <div className="flex items-center mb-2">
                    ...
                    <span className="font-medium text-gray-900 dark:text-white">{selectedToken.name}</span>
                  </div>
                ...
                <div className="mb-6">
                  <label className="block text-gray-700 dark:text-gray-300 mb-2 font-medium">Amount to deposit:</label>
                  ...
                  <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">Enter the amount you wish to deposit into DefiShield</p>
                </div>
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 mb-6">
                  <div className="flex items-start">
                    <Info className="text-blue-500 mr-3 flex-shrink-0 mt-0.5" size={20} />
                    <div>
                      <p className="text-sm text-gray-700 dark:text-blue-200 font-medium mb-1">Protected by DefiShield</p>
                      <p className="text-sm text-gray-600 dark:text-gray-300">
                        Your assets will be protected by our protocol and you can claim compensation in case of rug pulls or hacks.
                      </p>
                    </div>
                  </div>
                </div>
                <button
                  ...
                  className={`w-full p-4 rounded-xl font-medium text-lg transition-colors ${
                    isLoading ? "bg-gray-400 dark:bg-gray-600 text-white cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 text-white"
                  }`}
                >
            ...
            {depositStep === 2 && depositSuccess && (
              <div className="p-8">
                <div className="text-center py-8">
                  ...
                  <h2 className="text-2xl font-semibold mb-4 text-gray-900 dark:text-white">Deposit Successful!</h2>
                  ...
                  <button onClick={resetForm} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors">
          ...
          <div className="mt-8 bg-white dark:bg-slate-800 rounded-xl shadow-md border border-gray-100 dark:border-slate-700 p-6">
            ...
            <p className="text-gray-600 dark:text-gray-300 text-sm">
              If you're having trouble with your deposit or have any questions, our support team is here to help.
            </p>
            <div className="mt-4">
